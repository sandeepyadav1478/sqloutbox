...
- **Plateau, don't give up (embedded-consumer default).** At the cap the row keeps retrying at the cap interval — the library never *loses* a row. (A consumer may *choose* to stop polling / pause itself based on the signal — that's the consumer's call, §3.4.) **Amended by the standalone-hardening spec §7:** this plateau-forever behavior is now the `max_attempts=None` setting (autopulse's default). A standalone daemon sets a finite `max_attempts` so a poison head is *quarantined* to `outbox_dead_log` (audited, replayable) rather than blocking its namespace forever — quarantine relocates the row, it does not drop it.
...
- ✅ **Never-skip head + 2ⁿ backoff (cap configurable, default 64 min)**; the head plateau-retries at the cap interval. The library never *loses* a row. **Amended (2026-06-11) by the standalone-hardening spec §7:** in standalone-daemon mode (no consumer reading `health()`) the library may *quarantine* a head to an audited, replayable dead-letter (`outbox_dead_log`, move-not-delete) after `max_attempts` failed deliveries. `max_attempts=None` restores plateau-forever (the embedded-consumer default autopulse uses). Quarantine ≠ loss: the row is relocated, not dropped.
...
- ✅ **No policy thresholds for app-pause in the library** — counts exposed; consumer owns "300"/"10"/pause. **Amended (2026-06-11) by the standalone-hardening spec §7:** the library owns only its self-protection defaults `max_attempts` (auto-dead-letter) and `max_pending` (backpressure cap) for standalone-daemon mode; both are opt-out (`None` ⇒ exact embedded-consumer behavior). It still owns no *app-pause* threshold — the consumer owns "300"/"10"/pause.
